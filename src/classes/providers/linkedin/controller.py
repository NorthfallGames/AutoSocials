...
from classes.providers.linkedin.service import LinkedinService
...
class LinkedinMenuController(BaseAccountMenuController):
    ...
    def __init__(self) -> None:
        super().__init__(
            provider="linkedin",
            service_name="Linkedin Automator",
            account_fields=COMMON_ACCOUNT_FIELDS,
        )

    def _build_service(self, account: dict) -> LinkedinService:
    ...
    def run_account_session(self, account: dict) -> None:
        try:
            ...
            error(f"Failed to start Linkedin service: {exc}")
            return

        while True:
            print_banner()
            info("\n============ LINKEDIN OPTIONS ============", False)
    ...
    def _show_account_details(self, account: dict) -> None:
        success("Linkedin account details:")
